...
final class Feedback {
	 ...
	 * If they completed the form, give another 60 days and show it again.
	 *
	 * If they dimiss the form, give them 72 hours and then show the form again.
	 */
	...
	public function submit_feedback_to_nux( \WP_REST_Request $req ) {
		...
		$error = function() {
			...
			$dismissal_limit = isset( $this->nux_payload['limit_per_in_seconds'] ) ? $this->nux_payload['limit_per_in_seconds'] : 14 * DAY_IN_SECONDS;
			$this->dismiss( $dismissal_limit );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script(
			'wpaas-feedback',
			Plugin::assets_url( 'js/wpaas-feedback.min.js' ),
		...
		wp_localize_script(
			...
			[
				...
				'comment_length' => $this->get_comment_max_length( $this->nux_payload['rules']['comment'] ),
				'score_choices'  => $this->get_score_choices( $this->nux_payload['rules']['score'] ),
				'css'            => $css_url,
			]
		);
